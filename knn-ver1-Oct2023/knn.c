...
inline void copy_k_nearest(CLASS_ID_TYPE *dist_points_classification_id, DATA_TYPE *distance_dist_points, int num_points,
    ...
    int i = 0;
    for(i = 0; i < k; i++) {   // we only need the top k minimum distances
...
inline void copy_k_nearest_20(CLASS_ID_TYPE *dist_points_classification_id, DATA_TYPE *distance_dist_points, int num_points,
    ...
    for(i = 0; i < 20; i+=4) {   // we only need the top k minimum distances
       bp_classification_id[i] = dist_points_classification_id[i];
       bp_distance[i] = distance_dist_points[i];
...
inline void select_k_nearest_20(CLASS_ID_TYPE *dist_points_classification_id, DATA_TYPE *distance_dist_points, int num_points) {
 ...
 void get_k_NN(Point new_point, DATA_TYPE (*kp_features)[NUM_FEATURES], CLASS_ID_TYPE *kp_classification_id, int num_points,
    ...
    for (int i = 0; i < num_points; i++) {
        ...
        DATA_TYPE distances[4];
        for (int l = 0; l < 4; l++) {
            distances [l] = 0;
        }
        int j = 0;
        for (j = 0; j < num_features; j+=4) {
            DATA_TYPE diff1 = new_point.features[j] - kp_features[i][j];
            DATA_TYPE diff2 = new_point.features[j+1] - kp_features[i][j+1];
            DATA_TYPE diff3 = new_point.features[j+2] - kp_features[i][j+2];
            DATA_TYPE diff4 = new_point.features[j+3] - kp_features[i][j+3];
            
            distances[0] += diff1 * diff1;
            distances[1] += diff2 * diff2;
            distances[2] += diff3 * diff3;
            distances[3] += diff4 * diff4;
        ...
        distance += distances[0] + distances[1] + distances[2] + distances[3];
...
*
*	Note: it assumes that classes are identified from 0 toS
*	num_classes - 1.
*/
